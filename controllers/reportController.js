 ...
 */
const groupSalesByPeriod = (sales, groupBy) => {
  const grouped = {};
  ...
  try {
    ...
    res.json({
      summary: {
        totalCustomers,
        activeCustomers,
        newCustomers: customers.filter((c) => !from || new Date(c.createdAt) >= new Date(from)).length,
 ...
 */
const getCustomerRetentionReport = async (req, res) => {
  try {
